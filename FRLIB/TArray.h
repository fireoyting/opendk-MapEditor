...
public:
	...
	// File I/O
	//--------------------------------------------------------
	virtual bool		SaveToFile(ofstream& file);//继承类可重写此方法
	virtual bool		LoadFromFile(ifstream& file);//继承类可重写此方法
...
TArray<DataType, SizeType>::LoadFromFile(ifstream& file)
{
	...
	for (SizeType i = 0; i < m_Size; i++)
		m_pData[i].LoadFromFile(file);// GP的经典思路，通用方法。 然后传入类型。 实际该方法将执行各个子类型的LOADFROMFILE。
