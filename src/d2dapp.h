...
{
public:
    D2DApp(HINSTANCE hInstance, const std::wstring& windowName, int initWidth, int initHeight);
    virtual ~D2DApp();
...
protected:
    ...
    // Direct2D
    ComPtr<ID2D1Factory1> m_pd2dFactory;							// D2D工厂
    ComPtr<ID2D1Device> m_pD2DDevice;                                // D2D设备
    ComPtr<ID2D1DeviceContext> m_pd2dImmediateContext;				// D2D设备上下文
    ...
    ComPtr<ID2D1Bitmap1> m_pD2DTargetBimtap;			// 渲染目标位图
