...
D2DApp::D2DApp(HINSTANCE hInstance, const std::wstring& windowName, int initWidth, int initHeight)
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        else
        {
          ...
          /*      if (m_Timer.DeltaTime() < m_targetFrameTime)
                {
                    Sleep((DWORD)((m_targetFrameTime - m_Timer.DeltaTime()) * 1000));
                }*/
...
void D2DApp::OnResize()
{
    ...
    m_pd2dImmediateContext->SetTarget(nullptr);
    m_pD2DTargetBimtap.Reset();
    //m_pd3dImmediateContext->Flush();
    //ComPtr<IDXGISurface> backBuffer;
    //HR(m_pSwapChain->ResizeBuffers(2, m_ClientWidth, m_ClientHeight, DXGI_FORMAT_B8G8R8A8_UNORM, 0));	// 注意此处DXGI_FORMAT_B8G8R8A8_UNORM
    //HR(m_pSwapChain->GetBuffer(0, IID_PPV_ARGS(backBuffer.GetAddressOf())));
    ...
    ComPtr<IDXGISurface> backdxgiSurface;
    HR(m_pSwapChain->GetBuffer(0, __uuidof(IDXGISurface), reinterpret_cast<void**>(backdxgiSurface.GetAddressOf())));
    ...
    //bitmapProperties.pixelFormat.alphaMode = D2D1_ALPHA_MODE_IGNORE; // Bitmap alpha mode
    //bitmapProperties.bitmapOptions = D2D1_BITMAP_OPTIONS_TARGET | D2D1_BITMAP_OPTIONS_CANNOT_DRAW; // Bitmap options

  
    HR(m_pd2dImmediateContext->CreateBitmapFromDxgiSurface(
        backdxgiSurface.Get(), // DXGI surface
        &bitmapProperties, // Bitmap properties
        &m_pD2DTargetBimtap // D2D1 bitmap
    ));

        
    backdxgiSurface.Reset();
...
void D2DApp::SetFrameRate(float frameRate)
{

    m_targetFrameRate = frameRate;
    m_targetFrameTime = 1.0f / m_targetFrameRate;
